...
{
    private const string ChromiumVersion = "1-143.0.3650.75";
    ...
    private static Dictionary<string, string> Headers { get; } = new()
    {
        {
            "User-Agent",
        ...
        { "Accept", "*/*" },
        { "Cookie", "muid=" + GetCookieMuidValue().ToUpper() },
    };

    /// <summary>
    ///     Get a stream that contains the audio
    /// </summary>
    public static Stream GetAudioStream(this Voice voice, string text, PlaybackSettings? playbackSettings = null, CancellationToken ct = default)
    {
        ...
        _ = Task.Run(async () =>
            {
                try
                {
                    var targetStream = pipe.Writer.AsStream();

                    await voice.StreamTo(targetStream, text, playbackSettings, () => targetStream.Close(), error => pipe.Writer.CompleteAsync(error), ct);
                }
        ...
        return pipe.Reader.AsStream(); // 转换为 Stream 返回
    }
    ...
    /// </summary>
    public static async Task SaveAudioToFile(this Voice voice, string text, string path, PlaybackSettings? playbackSettings = null,
    ...
    /// </summary>
    public static async Task StreamText(this Voice voice, Stream targetStream, string text, PlaybackSettings? playbackSettings = null,
        CancellationToken ct = default)
    {
        var streaming = true;
        await voice.StreamTo(targetStream, text, playbackSettings, () => streaming = false, null, ct);
        ...
        CancellationToken ct = default)
    {
        ...
        var wss = new WebSocketClientWrapper(
            $"wss://speech.platform.bing.com/consumer/speech/synthesize/readaloud/edge/v1?TrustedClientToken=6A5AA1D4EAFF4E9FB37E23D68491D6F4&Sec-MS-GEC={GenerateSecMsGecToken()}&Sec-MS-GEC-Version={ChromiumVersion}",
            async (ws, data) =>
            {
                try
                {
        ...
        Array.ForEach(Headers.ToArray(), it => wss.AddHeader(it.Key, it.Value));
    ...
    private static string GetCookieMuidValue()
    {
